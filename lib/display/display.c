...
/*========== Display ==========*/
int Display_init(Display_t* disp, int screen_width, int screen_height) {
    ...
    disp->attrs.backing_pixel = 0x1C1B1D;    
    disp->attrs.border_pixel = 0x1C1B1D;

    // Set width and height of the framebuffer
    Display_setFrameBufferSize(disp, screen_width, screen_height);
    ...
    disp->window = XCreateSimpleWindow(disp->display, disp->parent, 0, 0, disp->width, disp->height, 0, 0x000000, 0x1C1B1D);
...
int Display_setFrameBufferSize(Display_t* disp, int screen_width, int screen_height) {
    int width, height;
